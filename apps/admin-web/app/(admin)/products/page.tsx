...
const pageSize = 20;
const categories = ['sofa', 'table', 'bed', 'appliance'];
export default async function ProductsPage({ searchParams }: { searchParams: Promise<{ search?: string; category?: string; offset?: string }> }) {
  ...
  return <><div className="heading-actions"><PageHeading title="商品管理" description="集中整理门店的家具与家电，查找商品更轻松。" /><Link className="primary-action" href="/products/new">新建商品</Link></div>
    <form className="search-form product-filters"><label htmlFor="product-search">搜索商品</label><input id="product-search" name="search" defaultValue={search} maxLength={100} placeholder="名称、SKU 或品牌" /><label className="visible-label" htmlFor="product-category">商品分类</label><input id="product-category" name="category" list="product-category-suggestions" defaultValue={category} maxLength={100} placeholder="输入任意分类" /><datalist id="product-category-suggestions">{categories.map(value => <option key={value} value={value} />)}</datalist><button type="submit">筛选</button></form>
