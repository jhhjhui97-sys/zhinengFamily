...
export default async function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><div className="detail-heading"><div><span className="eyebrow">客户详情</span><h1>{customer.name}</h1><p>查看客户资料和跟进信息。</p></div><div className="detail-actions"><Link className="primary-action" href={`/customers/${customer.id}/edit`}>编辑客户</Link><button className="secondary-action" disabled>创建设计方案（下一阶段开放）</button></div></div><section className="detail-card"><dl>{fields.map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</dl></section></>;
}
