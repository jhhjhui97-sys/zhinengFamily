...
export default async function ProjectsPage({ searchParams }: { searchParams: Promise<{ offset?: string }> }) {
  ...
  const customerNames = new Map<string, string>(); await Promise.all(result.items.map(async project => { try { customerNames.set(project.customer_id, (await getCustomer(project.customer_id)).name); } catch { customerNames.set(project.customer_id, '—'); } }));
