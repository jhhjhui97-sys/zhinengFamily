...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  catch (error) { if (error instanceof ApiError && error.status === 404) notFound(); if (error instanceof ApiError && error.status === 401) redirect('/api/auth/expired'); return <ResourceError error={error} href={`/projects/${id}`} />; }
  let customer; try { customer = await getCustomer(project.customer_id); } catch (error) { if (error instanceof ApiError && error.status === 401) redirect('/api/auth/expired'); return <ResourceError error={error} href={`/projects/${id}`} />; }
