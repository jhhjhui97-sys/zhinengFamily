...
export function ProjectForm({ project, initialCustomer }: { project?: DesignProject; initialCustomer?: Customer }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true);
    try { const response = await fetch(project ? `/api/projects/${project.id}` : '/api/projects', { method: project ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }); const body = await response.json(); if (response.status === 401) { router.replace('/login?expired=1'); router.refresh(); return; } if (!response.ok) throw new Error(typeof body.error === 'string' ? body.error : '服务暂时不可用，请稍后重试'); router.replace(`/projects/${body.id}`); router.refresh(); }
