...
def test_unity_sample_is_authoritative_two_bedroom_with_offline_furniture():
    ...
    assert (furniture.width_mm, furniture.depth_mm, furniture.height_mm) == (2400, 950, 850)
    assert (furniture.position.x, furniture.position.y, furniture.position.z) == (4000, 2000, 0)
...
def test_unity_export_is_current_and_preserves_unshown_fields():
    result = subprocess.run([sys.executable, str(ROOT / "tools/export_unity_sample.py"), "--check"], capture_output=True, text=True)
    assert result.returncode == 0, result.stderr + result.stdout
    source = json.loads((ROOT / "apps/admin-web/lib/two-bedroom.json").read_text(encoding="utf-8"))
...
def test_unity_export_check_does_not_modify_sample():
    ...
    result = subprocess.run([sys.executable, str(ROOT / "tools/export_unity_sample.py"), "--check"], capture_output=True)
