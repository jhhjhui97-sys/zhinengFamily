...
- [ ] 建立 npm 配置，安装锁定依赖；先写浏览器测试：`await page.goto('/'); await expect(page).toHaveURL(/login/); await page.getByRole('link',{name:'进入后台预览'}).click(); await expect(page).toHaveURL(/dashboard/);`。
- [ ] 运行 `npm test`，确认页面缺失导致失败。
- [ ] 最小实现根跳转、中文登录预览页和仪表盘入口，明确无真实认证。
- [ ] 运行登录测试、lint、typecheck，提交工具链与入口。
...
- [ ] 写真实页面测试：点击“客户管理”后期望 URL `/customers`、导航 aria-current=page、空状态“暂无客户”；商品和项目各有对应空状态。
- [ ] 写 iPad 测试：`await page.setViewportSize({width:820,height:1180}); expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBe(true);`；小屏菜单打开关闭、Esc 和焦点返回。
- [ ] 运行失败测试，确认布局、导航和空状态尚缺失。
- [ ] 实现共享布局、导航高亮、顶部占位、响应式样式与三个列表空状态。
- [ ] 运行浏览器回归，提交布局与页面。
...
- [ ] 记录 `npm install`、`npm run dev`、`npm run lint`、`npm run typecheck`、`npm run build`、`npm test`、`npm run start` 用法，明确预览和空状态限制。
- [ ] CI 在 Ubuntu 使用 Node 24、npm ci、lint/typecheck/build、Playwright Chromium 测试。
- [ ] 实际运行安装、所有检查、生产启动测试与 PC/iPad 可视检查；检查 diff 仅涉及前端范围及无敏感信息。
- [ ] 记录红绿灯和结果，分模块提交，推送功能分支；不创建 PR。
